...
use App\Modules\Follow_up_v2\Models\OAB\OAB_kuesioner_ipss;
use BS;
use DT;
use FORMAT;

trait OAB_kuesioner_ipssTrait {
    public function detail_oab_kuesioner_ipss($id, $follow_up_id): View
    {
        $pasien = $this->_allow_access($id);

        if (!OAB_kuesioner_ipss::where('pasien_id', $id)->where('follow_up_id', $follow_up_id)->exists()) {
            OAB_kuesioner_ipss::base_insert(['pasien_id' => $id, 'follow_up_id' => $follow_up_id]);
        }
    ...
    ): RedirectResponse
    {
        ...
        OAB_kuesioner_ipss::base_update_by_pasien_id_and_follow_up_id($data, $id, $follow_up_id);

        $this->flash_success_update($page_title);

        return redirect()->route(
            MODULE.'.detail_oab_kuesioner_ipss',
            ['id' => $id, 'follow_up_id' => $follow_up_id]
        );
    }
}
