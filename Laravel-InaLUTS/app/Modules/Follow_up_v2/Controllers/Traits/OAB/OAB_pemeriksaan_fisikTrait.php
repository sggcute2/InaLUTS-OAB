...
use App\Modules\Follow_up_v2\Models\OAB\OAB_pemeriksaan_fisik;
use BS;
use DT;
use FORMAT;

trait OAB_pemeriksaan_fisikTrait {
    public function detail_oab_pemeriksaan_fisik($id, $follow_up_id): View
    {
        $pasien = $this->_allow_access($id);

        if (!OAB_pemeriksaan_fisik::where('pasien_id', $id)->where('follow_up_id', $follow_up_id)->exists()) {
            OAB_pemeriksaan_fisik::base_insert(['pasien_id' => $id, 'follow_up_id' => $follow_up_id]);
        }
    ...
    ): RedirectResponse
    {
        ...
        OAB_pemeriksaan_fisik::base_update_by_pasien_id_and_follow_up_id($data, $id, $follow_up_id);

        $this->flash_success_update($page_title);

        return redirect()->route(
            MODULE.'.detail_oab_pemeriksaan_fisik',
            ['id' => $id, 'follow_up_id' => $follow_up_id]
        );
    }
}
