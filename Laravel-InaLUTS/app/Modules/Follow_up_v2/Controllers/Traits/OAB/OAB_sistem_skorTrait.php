...
use App\Modules\Follow_up_v2\Models\OAB\OAB_sistem_skor;
use BS;
use DT;
use FORMAT;

trait OAB_sistem_skorTrait {
    public function detail_oab_sistem_skor($id, $follow_up_id): View
    {
        $pasien = $this->_allow_access($id);

        if (!OAB_sistem_skor::where('pasien_id', $id)->where('follow_up_id', $follow_up_id)->exists()) {
            OAB_sistem_skor::base_insert(['pasien_id' => $id, 'follow_up_id' => $follow_up_id]);
        }
    ...
    ): RedirectResponse
    {
        ...
        OAB_sistem_skor::base_update_by_pasien_id_and_follow_up_id($request->all(), $id, $follow_up_id);

        $this->flash_success_update($page_title);

        return redirect()->route(
            MODULE.'.detail_oab_sistem_skor',
            ['id' => $id, 'follow_up_id' => $follow_up_id]
        );
    }
}
