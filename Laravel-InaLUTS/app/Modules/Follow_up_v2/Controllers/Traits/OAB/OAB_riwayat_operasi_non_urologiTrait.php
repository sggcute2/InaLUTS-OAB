...
use App\Modules\Follow_up_v2\Models\OAB\OAB_riwayat_operasi_non_urologi;
use BS;
use DT;
use FORMAT;

trait OAB_riwayat_operasi_non_urologiTrait {
    public function detail_oab_riwayat_operasi_non_urologi($id, $follow_up_id): View
    {
        $pasien = $this->_allow_access($id);

        if (!OAB_riwayat_operasi_non_urologi::where('pasien_id', $id)->where('follow_up_id', $follow_up_id)->exists()) {
            OAB_riwayat_operasi_non_urologi::base_insert(['pasien_id' => $id, 'follow_up_id' => $follow_up_id]);
        }
    ...
    ): RedirectResponse
    {
        ...
        OAB_riwayat_operasi_non_urologi::base_update_by_pasien_id_and_follow_up_id(
        ...
        return redirect()->route(
            MODULE.'.detail_oab_riwayat_operasi_non_urologi',
            ['id' => $id, 'follow_up_id' => $follow_up_id]
        );
    }
}
