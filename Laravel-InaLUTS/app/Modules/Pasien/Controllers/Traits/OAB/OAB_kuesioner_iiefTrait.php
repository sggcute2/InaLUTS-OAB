...
trait OAB_kuesioner_iiefTrait {
    public function detail_oab_kuesioner_iief($id): View
    {
        $pasien = $this->_allow_access($id);

        if (!OAB_kuesioner_iief::where('pasien_id', $id)->exists()) {
            OAB_kuesioner_iief::base_insert(['pasien_id' => $id]);
        }
    ...
    ): RedirectResponse
    {
        ...
        OAB_kuesioner_iief::base_update_by_pasien_id($data, $id);

        $this->flash_success_update($page_title);

        return redirect()->route(
            MODULE.'.detail_oab_kuesioner_iief', $id
        );
    }
}
