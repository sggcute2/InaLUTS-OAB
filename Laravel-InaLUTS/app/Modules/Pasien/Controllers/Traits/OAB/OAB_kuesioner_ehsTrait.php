...
trait OAB_kuesioner_ehsTrait {
    public function detail_oab_kuesioner_ehs($id): View
    {
        $pasien = $this->_allow_access($id);

        if (!OAB_kuesioner_ehs::where('pasien_id', $id)->exists()) {
            OAB_kuesioner_ehs::base_insert(['pasien_id' => $id]);
        }
    ...
    ): RedirectResponse
    {
        ...
        OAB_kuesioner_ehs::base_update_by_pasien_id($data, $id);

        $this->flash_success_update($page_title);

        return redirect()->route(
            MODULE.'.detail_oab_kuesioner_ehs', $id
        );
    }
}
